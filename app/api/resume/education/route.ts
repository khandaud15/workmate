...
export async function GET(req: Request) {
  try {
    ...
    console.log(`[EDUCATION API] Extracted timestamp from resumeId: ${timestamp || 'none'} (from ${resumeId})`);
    
    // First try to get the resume directly by the timestamp (which is the document ID in Firestore)
    let resumeDoc;
    if (timestamp) {
      console.log(`[EDUCATION API] Trying to find resume by timestamp ID: ${timestamp}`);
      ...
      if (resumeDoc.exists) {
        console.log(`[EDUCATION API] Found resume by timestamp ID: ${timestamp}`);
      }
    }
    
    // If not found by timestamp, try the full ID
    if (!resumeDoc || !resumeDoc.exists) {
      console.log(`[EDUCATION API] Trying to find resume by full ID: ${resumeId}`);
